...
function Game() {
  ...
  useEffect(() => {
    if (PlayersList[increment] == 'dead') {
      return setIncrement(increment < PlayersList.length - 1 ? increment + 1 : 0);
    }

    return setCurrentPlayer(PlayersList[increment].name);
  }, [increment]);

  useEffect(() => {
    if (currentPlayer == 'dead') setIncrement(increment < PlayersList.length - 1 ? increment + 1 : 0);
  }, [currentPlayer]);
